...
async function processChatCompletion(authSub, { agentId, messages, model, temperature: temperatureInput, max_tokens: maxTokensInput, sessionSkills, tools: requestTools, tool_choice: requestToolChoice, apiKeyId, apiKeyType }) {
  ...
  if (clientTools) {
    ...
    let e2bApiKey = null;
    if (e2bRow) {
      try { e2bApiKey = JSON.parse(e2bRow.data || '{}').valor || null; } catch {}
    }
  ...
  if (apiKeyId && apiKeyType === 'gratuita') {
    ...
    db.prepare('UPDATE api_keys SET monthly_tokens_used = ?, usage_month = ? WHERE id = ?')
      .run(previousUsage + totalTokens, month, apiKeyId);
  }
...
app.get('/api/keys', authMiddleware, (req, res) => {
  const rows = db.prepare(
    'SELECT id, name, key_prefix, last_used_at, revoked, created_at, key_type, monthly_tokens_used, usage_month FROM api_keys WHERE user_id = ? ORDER BY created_at DESC'
  ).all(req.auth.sub);
  const month = currentUsageMonth();
  res.json(rows.map(r => ({
    id: r.id,
...
app.post('/api/keys', authMiddleware, (req, res) => {
  ...
  db.prepare(
    'INSERT INTO api_keys (id, user_id, name, key_prefix, key_hash, key_type) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(id, req.auth.sub, name.trim(), keyPrefix, keyHash, keyType);
  ...
  res.status(201).json({
    ...
    key: fullKey,
    type: keyType,
    monthlyTokenLimit: keyType === 'gratuita' ? FREE_KEY_MONTHLY_TOKEN_LIMIT : null,
