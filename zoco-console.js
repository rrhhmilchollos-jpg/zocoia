...
// consolas de IA modernas), conectados al motor local de Ollama y a los ejecutores reales:
//
...
//      la API de Vercel, reutilizando la configuración
...
export function registerConsoleRoutes({ app, db, authMiddleware, uuidv4, processChatCompletion }) {
  ...
  // {"custom_id":"x","prompt":"..."} — formato estándar de batches).
  ...
  // ══ IMPLEMENTACIONES ═══════════════════════════════════════════════════
  // Conectadas a la API real de Vercel. El token
  // se toma de: variables del entorno ACTIVO del usuario (
  // VERCEL_TOKEN) → variables de entorno del servidor. Cada acción queda
  // registrada en deployments_log.

  // URL base de tu instancia de Coolify (server-wide, no por usuario — es tu único servidor).
  const COOLIFY_BASE_URL = process.env.COOLIFY_BASE_URL || 'http://165.227.130.179:8000';
  ...
  function resolveDeployToken(userId, provider) {
    ...
    if (provider === 'coolify') return vars.COOLIFY_API_TOKEN || process.env.COOLIFY_API_TOKEN || null;
    return null;
  }
  ...
  app.get('/api/implementaciones', authMiddleware, (req, res) => {
    ...
    res.json({
      ...
      coolifyConfigured: !!resolveDeployToken(req.auth.sub, 'coolify'),
    });
  });

  // Ejecutar acción de despliegue: { provider: 'vercel', action: 'redeploy', targetId }
  app.post('/api/implementaciones/accion', authMiddleware, async (req, res) => {
    const { provider, action, targetId } = req.body || {};
    if (provider !== 'vercel') return res.status(400).json({ error: 'provider debe ser vercel' });
    if (action !== 'redeploy') return res.status(400).json({ error: 'Acción no soportada (disponible: redeploy)' });
    if (!targetId || !String(targetId).trim()) return res.status(400).json({ error: 'targetId (proyecto) es obligatorio' });
