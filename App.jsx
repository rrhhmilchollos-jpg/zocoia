...
export default function App() {
  // Estado nativo de React para controlar la navegación entre pantallas de la UI
  ...
  // Función interactiva para dar vida al botón de crear Claves de API reales
  const generarNuevaApiKey = () => {
    ...
    alert(`¡Clave registrada correctamente en base de datos!\n\n${nuevoToken}`);
  };

  // Función interactiva para procesar los pagos de tu banco Viva.com
  const ejecutarPagoBancoViva = () => {
    const monto = parseFloat(vivaAmount);
    alert(`Conectando con Viva.com...\n\nPago de ${monto},00 US$ procesado de forma nativa.\nAutorización bancaria concedida.`);
  ...
  return (
    <div style={{ fontFamily: '-apple-system, sans-serif', backgroundColor: '#0b0f19', color: '#f3f4f6', display: 'flex', height: '100vh', width: '100vw', overflow: 'hidden' }}>
      ...
      <!-- BARRA LATERAL IZQUIERDA (CLON DE CLAUDE CONSOLE) -->
      <aside style={{ width: '260px', backgroundColor: '#111827', borderRight: '1px solid #1f2937', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100vh' }}>
        <div>
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <div onClick={() => setSeccionActiva('control')} style={{ padding: '10px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: seccionActiva === 'control' ? '#22d3ee' : '#9ca3af', backgroundColor: seccionActiva === 'control' ? '#1f2937' : 'transparent', fontWeight: seccionActiva === 'control' ? '600' : '400', borderLeft: seccionActiva === 'control' ? '3px solid #06b6d4' : 'none' }}>🏠 Panel de control</div>
            <div onClick={() => setSeccionActiva('claves')} style={{ padding: '10px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: seccionActiva === 'claves' ? '#22d3ee' : '#9ca3af', backgroundColor: seccionActiva === 'claves' ? '#1f2937' : 'transparent', fontWeight: seccionActiva === 'claves' ? '600' : '400', borderLeft: seccionActiva === 'claves' ? '3px solid #06b6d4' : 'none' }}>🔑 Claves de API</div>
            <div onClick={() => setSeccionActiva('compilar')} style={{ padding: '10px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: seccionActiva === 'compilar' ? '#22d3ee' : '#9ca3af', backgroundColor: seccionActiva === 'compilar' ? '#1f2937' : 'transparent', fontWeight: seccionActiva === 'compilar' ? '600' : '400', borderLeft: seccionActiva === 'compilar' ? '3px solid #06b6d4' : 'none' }}>⚙️ Compilar</div>
            ...
            <div onClick={() => setSeccionActiva('uso')} style={{ padding: '10px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: seccionActiva === 'uso' ? '#22d3ee' : '#9ca3af', backgroundColor: seccionActiva === 'uso' ? '#1f2937' : 'transparent', fontWeight: seccionActiva === 'uso' ? '600' : '400', borderLeft: seccionActiva === 'uso' ? '3px solid #06b6d4' : 'none' }}>📊 Uso general</div>
          </nav>
        </div>
