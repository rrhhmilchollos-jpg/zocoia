...
export default function Dashboard() {
  ...
  const handleEditAgent = (agent: Recurso) => {
    setEditingAgent(agent);
    setAgentModalOpen(true);
  };

  const onAgentSuccess = () => {
    load('/api/resources?type=agente', setAgentes);
    load('/api/billing/summary', setBilling);
  };
...
{/* ── CLAVES API ── */}
            ...
            <div className="max-w-5xl">
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-gray-500 text-xs mt-1 uppercase tracking-widest font-bold">Gestión de acceso programático</p>
                </div>
                <button 
                  onClick={handleCreateKey} 
                  className="bg-purple-600 hover:bg-purple-500 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-purple-500/20 transition-all"
                >
                  + Nueva Clave de API
                </button>
              </div>
                ...
                >
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleEditAgent(a); }} className="text-gray-500 hover:text-purple-400 text-xs border border-[#333] px-2 py-1 rounded-lg">⚙️ Configurar</button>
          ...
          {['archivo','habilidad','lote'].includes(activeTab) && (() => {
            if (activeTab === 'habilidad') return <ToolboxPanel />;
