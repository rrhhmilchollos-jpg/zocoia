...
async function coolifyFetch(endpoint, init = {}) {
  ...
  if (!res.ok) {
    const message = json?.message || `Coolify API error: ${res.statusText}`;
...
export async function getApplication(applicationUuid) {
  return coolifyFetch(`/applications/${applicationUuid}`);
}

/** Busca la aplicación en Coolify cuyo repo de GitHub coincide con el que trabajó el agente. */
export async function findApplicationByRepo(owner, repoName) {
  const apps = await coolifyFetch('/applications');
  const needle = `${owner}/${repoName}`.toLowerCase();
  return apps.find((app) => (app.git_repository || '').toLowerCase().includes(needle)) || null;
}

/** Dispara un redespliegue de la aplicación indicada. */
export async function triggerDeploy(applicationUuid) {
  if (!applicationUuid) {
    throw new CoolifyToolError(
      'applicationUuid no configurado: no se puede desplegar.'
    );
  }
  const result = await coolifyFetch(`/deploy?uuid=${encodeURIComponent(applicationUuid)}`, {
    method: 'GET',
  });
...
export async function getDeploymentStatus(deploymentUuid) {
  return coolifyFetch(`/deployments/${deploymentUuid}`);
}

/** Espera (con polling) a que un despliegue termine, con timeout de seguridad. */
export async function waitForDeployment(deploymentUuid, { timeoutMs = 5 * 60_000, intervalMs = 5_000 } = {}) {
