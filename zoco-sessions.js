...
// (con sesiones persistentes) sobre la base existente de Zoco IA, sin
...
//   1) SESIONES  — conversaciones persistentes en servidor:
...
export function registerSessionRoutes({ app, db, authMiddleware, uuidv4, serverSecret, processChatCompletion }) {
  ...
  app.get('/api/sesiones', authMiddleware, (req, res) => {
    ...
    // último mensaje como preview de la lista de chats
  ...
  app.post('/api/sesiones/:id/mensajes', authMiddleware, async (req, res) => {
    try {
      ...
      // (el clip añade contexto al hilo de la conversación).
      ...
      // Autotitular la conversación con el primer mensaje.
