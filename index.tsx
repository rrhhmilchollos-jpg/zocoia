...
// Interfaz para tipar las llaves comerciales en TypeScript
interface ApiKey {
  nombre: string;
  token: string;
  limite: string;
...
export default function App() {
  // Estados nativos de React con tipado estricto
  const [seccionActiva, setSeccionActiva] = useState<string>('control');
  const [saldo, setSaldo] = useState<number>(-1.73);
  const [vivaAmount, setVivaAmount] = useState<string>('50');
  const [bannerText, setBannerText] = useState<string>('ℹ️ El acceso a tus 11 agentes de software locales ha sido restaurado con éxito.');
